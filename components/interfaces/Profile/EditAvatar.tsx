...
export default function EditAvatar(
  { avatarUrl, fullName, email }: { avatarUrl: string | null, fullName: string | null, email: string }
) {
  const [state, formAction, isPending] = useActionState<EditAvatarActionState, FormData>(
    updateAvatarAction, {
      isSuccess: false,
      error: null,
      avatarUrl,
  ...
  return (
    <form action={formAction} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex flex-row items-center justify-between gap-4 rounded-lg p-6">
